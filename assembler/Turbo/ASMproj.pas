...
{ The data type "table" is used for both the Symbol Table and the Opcode Table.
  ...
  4213, then the Symbol Table node that has key='[1]' will have data=4213. }
    ...
    node = record
              key: strng ;
	      data: strng ;  { data changed from 'integer' to 'strng' }
	      next: pointer
	   end ;
    table = pointer ;
...
function strcpy(S: strng; Slen: integer): strng ;
BEGIN
     if (S <> null) then
	BEGIN
...
procedure update(var T: table; k, d: strng) ;
var
   p: pointer ;
BEGIN
     if T = Nil then
...
procedure lookup(T: table; k: strng; var answer: strng) ;
BEGIN
     if T = Nil then
...
procedure read_asm_line(var F: markfile; var labl, mnemonic, operand: strng) ;
var
   S, first: string ;
...
BEGIN
   ...
   if ( S_len > (mnem + 2) ) and (S[mnem + 3] <> space) then
      mnemonic := mnemonic + S[mnem + 3] ;
   chop(mnemonic) ;
   op := mnem + length(mnemonic) ;
   while ( op <= S_len ) and ( S[op] = space ) do inc(op) ;
   if ( op > S_len ) then
      operand := null
   else
...
var
   ...
   labl, mnemonic, operand, answer, A : strng ;
...
BEGIN
     ...
     while ( not EOF(infile) ) and ( lines < maxStatements ) do
     	BEGIN
...
{testing}  writeln('updating SymbolTable: labl = ', labl, ' / address = ', address) ;
	...
	END;  { while not EOF }
     ...
     if ( lines = maxStatements ) and ( not EOF(infile) ) then
	BEGIN
