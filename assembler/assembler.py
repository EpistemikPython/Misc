...
def create_empty(var T: table):

     T := nil
  # proc create_empty


def chop(var S: string):
var
   Slen: integer
...
def write_table_line(var F: markfile, var T: table):
var
   num_add : integer
	...
	writeln(F)
	  # else
  # proc write_table_line

# update - given values for "T", "k", and "d", this def adds an entry to "T" that
  # has "k" as its key part and "d" as its data part.  This def may assume that "T"
...
def update(var T: table k, d: str):
var
   p: pointer
...
def create_opcode_table(var opcode_table: table):
var
   opfile: markfile
   s: string
   opkey, opdata : str
...
def assemble(var infile, outfile : markfile  name: string):
var
...
#testing
        writeln('Line read.  labl: ', labl, ' mnem: ', mnemonic, ' op: ', operand)

     	if ( labl <> null ) then

#testing
            writeln('updating SymbolTable: labl = ', labl, ' / address = ', address)
