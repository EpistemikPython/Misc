...
# -- get the frequency and number of appearances of each letter in words of a specified length from a word list
#
...
def run():
    """Get the frequency and number of appearances of each letter in words of a specified length from a word list."""
    freqs = dict.fromkeys(string.ascii_uppercase, 0)
    appearances = dict.fromkeys(string.ascii_uppercase, 0)
    word_count = 0
    letter_count = 0
    ...
    for val in sorted(lett_freqs, reverse = True):
        lgr.info(f"\t{lett_freqs[val]}: {val}")
        sorted_freqs[lett_freqs[val]] = val
    ...
    for key in appearances.keys():
        ...
        # hopefully there are not two appearance totals exactly the same
    ...
    if save_option:
        ...
        save_dict = {"Letter frequencies":sorted_freqs, "Letter appearances":sorted_appears}
        saved_file = save_to_json(f"{min_size}-{max_size}_letter-freqs-appears", save_dict)
        lgr.info(f"\n\t Saved results to '{saved_file}'")

def set_args():
    arg_parser = ArgumentParser(description = "Get the save-to-file, file path, minimum and maximum word-size options:",
